...
func (l *Loop) SetTimer(fn *spidermonkey.Object, delay time.Duration, repeat bool) int64 {
	...
	// Node clamps a delay above TIMEOUT_MAX (2^31-1 ms, ~24.8 days) down to 1ms
	// (with a TimeoutOverflowWarning). Without this an overflowing setTimeout
	// effectively never fires instead of firing ~immediately.
	if delay > 2147483647*time.Millisecond {
...
// ClearTimers frees and removes every armed timer and immediate WITHOUT touching
// the loop's pending/posts accounting. The request-boundary reset calls this
// BEFORE draining in-flight async work, so a leftover setInterval/setTimeout
// can't fire mid-drain and spawn new work (e.g. a fetch) whose goroutine would
// survive into the next request on a reused instance and corrupt its pending
// accounting. Timer liveness is tracked by presence in l.timers (hasRefTimer),
// not by pending/unrefPending, so emptying these leaves the accounting intact.
// Must be called when the loop is not running (no callback in flight), like Reset.
func (l *Loop) ClearTimers() {
	l.mu.Lock()
	timers := l.timers
	l.timers = map[int64]*timer{}
	imms := l.immediates
	...
	l.dueBatch = nil
	l.mu.Unlock()
	// Idempotent freeFn: an id present in both l.timers and dueBatch frees once.
	for _, t := range timers {
		t.freeFn()
	}
	for _, t := range due {
