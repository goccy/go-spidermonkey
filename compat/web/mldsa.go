...
func (s *subtleAPI) opMLDSAImport(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
	...
	case "jwk":
		...
		if j.Kty != "AKP" {
			return subtleErr("DataError: ML-DSA JWK must have kty AKP"), nil
		}
	...
	return subtleErr("NotSupportedError: unsupported ML-DSA key format " + format), nil
}
...
func (s *subtleAPI) opMLDSAExport(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
	...
	if err != nil || sk.mldsa == nil {
		return subtleErr("InvalidAccessError: not an ML-DSA key"), nil
	}
	k := sk.mldsa
	...
	return subtleErr("NotSupportedError: unsupported ML-DSA export format"), nil
}
...
func (s *subtleAPI) opMLDSASign(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
	...
	if err != nil || sk.mldsa == nil || !sk.mldsa.private() {
		return subtleErr("InvalidAccessError: signing needs an ML-DSA private key"), nil
	}
	...
	if len(ctx) > mldsaContextMax {
		return subtleErr(fmt.Sprintf("OperationError: ML-DSA context must be at most %d bytes", mldsaContextMax)), nil
	}
	k := sk.mldsa
