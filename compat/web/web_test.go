...
// drainWeb runs the event loop to completion — fetch is asynchronous, so a
// fetch()'s .then chain only settles once the loop drives it. After Wait returns,
// state written by permission hooks (on the transport goroutine) is visible via
// the loop's Post happens-before, so it can be read race-free.
func drainWeb(t *testing.T, w *web.Web) {
	t.Helper()
...
// TestURLSearchParamsIterableAndTwoArg covers the URLSearchParams iterable
// constructor (Map/Set) and the two-argument delete/has forms, plus reportError.
func TestURLSearchParamsIterableAndTwoArg(t *testing.T) {
	js, _ := newWeb(t, spidermonkey.Config{})
	if got := evalString(t, js, `new URLSearchParams(new Map([["a","1"],["b","2"]])).toString()`); got != "a=1&b=2" {
		t.Errorf("URLSearchParams(Map) = %q, want a=1&b=2", got)
	}
	if got := evalString(t, js, `(() => { const p = new URLSearchParams("a=1&a=2"); p.delete("a","2"); return p.toString(); })()`); got != "a=1" {
		t.Errorf("delete(name,value) = %q, want a=1", got)
	}
	if got := evalString(t, js, `(() => { const p = new URLSearchParams("a=1&a=2"); return p.has("a","3") + "|" + p.has("a","2"); })()`); got != "false|true" {
		t.Errorf("has(name,value) = %q, want false|true", got)
	}
	if got := evalString(t, js, `typeof globalThis.reportError`); got != "function" {
		t.Errorf("reportError = %q, want function", got)
	}
}

// TestURLSearchParamsStarLiteral verifies * is left literal (WHATWG) while ~ is
// percent-encoded.
func TestURLSearchParamsStarLiteral(t *testing.T) {
	js, _ := newWeb(t, spidermonkey.Config{})
	if got := evalString(t, js, `new URLSearchParams([["x","a*b~c"]]).toString()`); got != "x=a*b%7Ec" {
		t.Errorf("URLSearchParams serialization = %q, want x=a*b%%7Ec", got)
	}
}

// TestAbortSignalAny verifies AbortSignal.any([...]) aborts as soon as any source
// signal aborts (the standard combine-user-abort-with-timeout fetch pattern).
func TestAbortSignalAny(t *testing.T) {
	js, _ := newWeb(t, spidermonkey.Config{})
	got := evalString(t, js, `
		(() => {
			if (typeof AbortSignal.any !== "function") return "no-any";
			const c1 = new AbortController(), c2 = new AbortController();
			const any = AbortSignal.any([c1.signal, c2.signal]);
			const before = any.aborted;
			c2.abort("boom");
			return before + "|" + any.aborted + "|" + String(any.reason);
		})()
	`)
	if got != "false|true|boom" {
		t.Errorf("AbortSignal.any = %q, want false|true|boom", got)
	}
	// An already-aborted source aborts the combined signal immediately.
	if g := evalString(t, js, `
		(() => {
			const a = AbortSignal.abort("pre");
			const any = AbortSignal.any([a]);
			return any.aborted + "|" + String(any.reason);
		})()
	`); g != "true|pre" {
		t.Errorf("AbortSignal.any with pre-aborted source = %q, want true|pre", g)
	}
}
