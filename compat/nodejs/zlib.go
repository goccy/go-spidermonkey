...
// zlibRun is the shared codec (compat/internal/compress): the same one-shot
// transforms back the WHATWG CompressionStream in compat/web.
func zlibRun(method string, data []byte) ([]byte, error) { return compress.Run(method, data) }
...
func (d *zlibDecomp) run(method string) {
	...
	if terminal == nil {
		...
		for {
			...
			if n > 0 {
				total += int64(n)
				if total > compress.MaxOutput {
					terminal = fmt.Errorf("decompressed output exceeds %d bytes", compress.MaxOutput)
					break
				}
				d.mu.Lock()
