...
type httpPending struct {
	...
	reqID    int64 // the guest-facing request id (for abort notification)
	rt       *Runtime
...
// write callbacks (so the guest Writable is not left hanging), and finishes. A
// non-nil cause means the client went away before the guest ended the response
// (ctx cancellation) rather than a normal end — surface that to the guest so
// res.on('close') / req.on('close'/'aborted') fire (the canonical SSE / long-poll
// cancellation hooks), which a bodyless request would otherwise never see.
func (p *httpPending) shutdown(cause error) {
	p.mu.Lock()
	already := p.closed
	p.closed = true
	leftover := p.queue
	...
	if cause != nil && !already && p.rt.httpAborted != nil {
		reqID := p.reqID
		p.rt.loop.Post(func() error {
			p.rt.httpAborted.Call(spidermonkey.ValueOf(reqID))
			return nil
		})
	}
	p.finish()
...
// HTTPHandler returns an http.Handler for the single HTTP server the guest has
// started with http.createServer(...).listen(...), so an embedder can serve
// that Node app through its own server (or httptest) instead of reaching the
// guest's listener over a socket. It reports false until exactly one server is
// running (dispatch is independent of the guest's own listener).
...
func (rt *Runtime) opHTTPClose(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
	...
	if !ok {
		// Already closed (or never listened): the listen's keep-alive ref was
		// released on the first close, so don't DonePending again — just report.
		if cb != nil {
	...
	// Graceful shutdown: stop accepting new connections but let in-flight
	// requests finish (Node semantics), rather than Close()'s hard abort that
	// truncates responses. srv.Shutdown blocks until every active ServeHTTP —
	// which waits for the guest to end its response — returns. The listen's
	// AddPending is held until then, keeping the loop alive so those handlers can
	// run, and the guest close callback fires only once the drain completes.
	go func() {
...
func (rt *Runtime) opHTTPWrite(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
	...
	if p == nil {
		// The request is gone (client disconnected mid-response): free the data
		// Buffer arg the normal path would consume, then ack the write.
