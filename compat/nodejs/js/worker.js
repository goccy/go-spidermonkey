...
(() => {
	...
	function scheduleTimer(fn, ms, repeat, args) {
		...
		const fire = () => {
			...
			try {
				...
				if (repeat && liveTimers.has(id)) arm();
				else {
					liveTimers.delete(id);
					// A non-repeating timer just fired and left no timers behind: the
					// worker may now be idle. Schedule an idle check (which re-tests the
					// predicate as a macrotask); never exit synchronously from here.
					if (liveTimers.size === 0 && globalThis.__wt_idleCheck) globalThis.__wt_idleCheck();
				}
			}
		};
		const arm = () => {
	...
	// user source runs.
	const init = A.recv();
	// The spawn was aborted before it completed (e.g. the workerData clone
	// failed): leave immediately without running the user source, so the agent
	// goroutine isn't orphaned blocked here.
	if (init && init.__terminate__) { A.leaving(); return; }
	...
	function makePort() {
		...
		// addEventListener wraps the user fn (to pass a {data} event), so
		// removeEventListener needs the original->wrapper mapping to remove it.
		const evlWrap = new Map();
		// After the last parentPort 'message' listener is removed, the worker may
		// have become idle — schedule an idle check (never exit synchronously from
		// inside a listener callback).
		const maybeIdleAfterOff = () => {
			if (handlers.message.length === 0 && globalThis.__wt_idleCheck) globalThis.__wt_idleCheck();
		};
		return {
			// The number of live 'message' listeners keeping this worker alive.
			_messageListenerCount() { return handlers.message.length; },
			postMessage(v, transferList) {
				// A.post clones the message synchronously; after the clone has read the
				// bytes, detach each transferred ArrayBuffer on this (sender) side via
				// the engine's real transfer, so the sender's byteLength drops to 0
				// while the receiver keeps a copy of the bytes.
				A.post({ __wt_msg: true, data: v });
			...
			off(type, fn) {
				const l = handlers[type];
				if (l) { const i = l.indexOf(fn); if (i >= 0) l.splice(i, 1); }
				if (type === "message") maybeIdleAfterOff();
				return this;
			},
			addEventListener(type, fn) {
				const t = type === "message" ? "message" : type;
				const w = (data) => fn({ data });
				evlWrap.set(fn, w);
				return this.on(t, w);
			},
			removeEventListener(type, fn) {
				const t = type === "message" ? "message" : type;
				const w = evlWrap.get(fn);
				if (w) { evlWrap.delete(fn); return this.off(t, w); }
				return this;
			},
	...
	// Guarded so it fires at most once — an idle auto-exit must never double-post
	// after an explicit process.exit()/parentPort.close()/uncaught-throw exit.
	let exiting = false;
	const exitWorker = (code) => { if (exiting) return; exiting = true; A.post({ __wt_exit: Number(code) || 0 }); A.leaving(); };
	globalThis.process ??= {
		exit: exitWorker,
	...
	// ---- idle auto-exit (Node parity) ----
	// A worker with an empty event loop exits on its own with code 0. Here the
	// loop is "empty" when there are no pending timers/immediates AND no
	// 'message' listener on parentPort keeping it alive. (A pending-but-unbacked
	// Promise does not keep Node alive, so it need not keep the worker alive.)
	const isIdle = () => liveTimers.size === 0 && parentPort._messageListenerCount() === 0;
	// Schedule the idle test as a genuine MACROTASK, NOT registered in liveTimers,
	// so it runs only AFTER the microtask queue has fully drained — this way a
	// listener/timer that the source installs after several microtask hops (an
	// `await`/chained `.then` init) is visible before we decide to exit. A 0ms
	// Atomics.waitAsync returns synchronously ({async:false}), which would
	// degrade to a microtask and fire mid-chain; a >=1ms wait yields a real
	// async job that the engine only runs once microtasks are exhausted.
	globalThis.__wt_idleCheck = () => {
		if (exiting) return;
		const sab = new Int32Array(new SharedArrayBuffer(4));
		const w = Atomics.waitAsync(sab, 0, 0, 1);
		const run = () => { if (!exiting && isIdle()) exitWorker(0); };
		if (w.async) w.value.then(run); else Promise.resolve().then(run);
	};
	...
	// Post the error as a structured {name, message, stack} so the main thread can
	// reconstruct an Error with the ORIGINAL message/name/stack (a flattened string
	// would lose err.message === "boom"). See reviveWorkerError on the main side.
	globalThis.__wt_reportError = (e) => {
		A.post({ __wt_error: e instanceof Error ? { name: e.name, message: e.message, stack: e.stack || "" } : { message: String(e) } });
		exitWorker(1);
	};
	...
	globalThis.__deliver__ = (msg) => {
		...
		if (msg && msg.__wt_msg) {
			...
			catch (e) { A.post({ __wt_error: e instanceof Error ? { name: e.name, message: e.message, stack: e.stack || "" } : { message: String(e) } }); }
		}
	};
})();
