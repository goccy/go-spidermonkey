...
func (rt *Runtime) opSignalWatch(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
	...
	rt.io.sigOnce.Do(func() {
		...
		// A signal watcher does NOT keep the loop alive (Node: a lone
		// process.on('SIGTERM', …) still lets the process exit). So we take NO
		// pending for it — a delivered signal is still dispatched while the loop
		// runs for other reasons, but the watcher alone cannot pin it. The
		// goroutine is reaped by closeIO closing stop; sigCh is never closed (a
		// late delivery after Stop would panic on a closed channel), it's left
		// for GC, and no handle is freed here (js.Close frees everything at
		// teardown). Taking no pending means there is nothing to release, so no
		// accounting can underflow.
		go func() {
			for {
				select {
				case s := <-ch:
