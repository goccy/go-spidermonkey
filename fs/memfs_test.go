...
func TestWriteReadRoundTrip(t *testing.T) {
	...
	got, err := iofs.ReadFile(fsys, "f.txt")
	if err != nil {
		t.Fatal(err)
	}
	if string(got) != "first" {
	...
	f.Close()
	got, _ = iofs.ReadFile(fsys, "f.txt")
...
func TestOpenFileFlags(t *testing.T) {
	...
	if _, err := fsys.OpenFile("missing.txt", os.O_WRONLY, 0o644); !errors.Is(err, iofs.ErrNotExist) {
		t.Errorf("write without O_CREATE: err = %v, want ErrNotExist", err)
	}
	if err := fsys.WriteFile("f.txt", []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	if _, err := fsys.OpenFile("f.txt", os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0o644); !errors.Is(err, iofs.ErrExist) {
		t.Errorf("O_EXCL on existing: err = %v, want ErrExist", err)
	}
	if _, err := fsys.OpenFile("no/parent.txt", os.O_WRONLY|os.O_CREATE, 0o644); !errors.Is(err, iofs.ErrNotExist) {
...
func TestMkdirRemove(t *testing.T) {
	...
	if err := fsys.Mkdir("d", 0o755); !errors.Is(err, iofs.ErrExist) {
		t.Errorf("second Mkdir: err = %v, want ErrExist", err)
	}
	if err := fsys.Mkdir("x/y", 0o755); !errors.Is(err, iofs.ErrNotExist) {
	...
	if _, err := fsys.Open("d"); !errors.Is(err, iofs.ErrNotExist) {
...
func TestRenameMovesSubtree(t *testing.T) {
	...
	got, err := iofs.ReadFile(fsys, "new/sub/f.txt")
	if err != nil {
		t.Fatal(err)
	}
	if string(got) != "data" {
	...
	if _, err := fsys.Open("old/sub/f.txt"); !errors.Is(err, iofs.ErrNotExist) {
